...
class AddressBook(): 
    ...
    def addRecord(self):  
        ...
        while True:
            ...
            try:
                ...
                if len(address) >= 70:
                    print("length of address should be less than 70")
                    continue
    ...
    # this function is used for printing data
    def printData(self):  
    ...
    # sort function is used for sorting the data in json file
    def sort(self):  

        array = []
        data = self.data  

        # first name is appended in array
        for i in range(len(self.data)): 
            array.append((self.data[i]["first_name"]))
             #names are sorted
        name_sort = sorted(array) 

        # this empty array is used for storing sorted file
        sorteddata = []  

        # nested loop is used for matching sorted names to file
        for i in name_sort:  
            
            for j in range(len(data)):
                
                if i == data[j]["first_name"]:
                    # if name matches then data is appended
                    sorteddata.append(data[j])  

        # now we have swapped data with sorted data
        self.data = sorteddata  
        # here we have return sorted data
        return sorteddata  

    def dumping(self, filename):  

        with open('/home/user/Desktop/newfolder/Fellowship/addressbook/details.json', 'w') as f:
            json.dump(self.data, f, indent=2)

    def onlyNames(self): 
         # printing names from the file
        data = self.data
        for i in range(len(data)):
